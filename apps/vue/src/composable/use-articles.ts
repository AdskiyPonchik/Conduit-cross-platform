...
export function useArticles() {
  const { articlesType, tag, username, searchQuery, metaChanged } = useArticlesMeta()
  ...
  async function fetchArticles(): Promise<void> {
    ...
    if (articlesType.value === 'my-feed') {
      ...
      responsePromise = api.search.searchArticles({ query: searchQuery.value.replace(/ /g, '+'), ...pageToOffset(page.value) })
        .then(res => res.data)
    }
...
export type ArticlesType = 'global-feed' | 'my-feed' | 'tag-feed' | 'user-feed' | 'user-favorites-feed' | 'search-feed'

export const articlesTypes: ArticlesType[] = ['global-feed', 'my-feed', 'tag-feed', 'user-feed', 'user-favorites-feed', 'search-feed']
...
function useArticlesMeta(): UseArticlesMetaReturn {
  ...
  watch(
    () => route.query.q,
    qParam => {
      searchQuery.value = typeof qParam === 'string' ? qParam : ''
    },
    { immediate: true },
  )
  ...
  return {
    ...
    metaChanged: computed(() => `${articlesType.value}-${username.value}-${tag.value}-${searchQuery.value}`),
  }
}
